
-- Add missing class mapping for Мастер Целитель
INSERT INTO card_class_mappings (card_name, card_type, class_name, created_by_wallet_address)
VALUES ('Мастер Целитель', 'hero', 'Мастер Целитель', 'mr_bruts.tg')
ON CONFLICT DO NOTHING;

-- Recalculate all card stats with proper multipliers
DO $$
DECLARE
  card_record RECORD;
  base_health INTEGER;
  base_defense INTEGER;
  base_power INTEGER;
  base_magic INTEGER;
  rarity_mult NUMERIC;
  class_mult NUMERIC;
  final_health INTEGER;
  final_defense INTEGER;
  final_power INTEGER;
  final_magic INTEGER;
  card_class TEXT;
BEGIN
  -- Loop through all hero and dragon cards
  FOR card_record IN 
    SELECT 
      ci.id, 
      ci.card_data, 
      ci.card_type,
      ci.max_health as old_health,
      ci.max_defense as old_defense,
      ci.max_power as old_power,
      ci.max_magic as old_magic
    FROM card_instances ci
    WHERE ci.card_type IN ('hero', 'dragon')
  LOOP
    -- Get base stats based on card type
    IF card_record.card_type = 'hero' THEN
      SELECT health, defense, power, magic 
      INTO base_health, base_defense, base_power, base_magic
      FROM hero_base_stats LIMIT 1;
    ELSE
      SELECT health, defense, power, magic 
      INTO base_health, base_defense, base_power, base_magic
      FROM dragon_base_stats LIMIT 1;
    END IF;

    -- Get rarity multiplier
    SELECT multiplier INTO rarity_mult
    FROM rarity_multipliers
    WHERE rarity = CAST(card_record.card_data->>'rarity' AS INTEGER);
    
    IF rarity_mult IS NULL THEN
      rarity_mult := 1.0;
    END IF;

    -- Get class name from card_class_mappings
    SELECT class_name INTO card_class
    FROM card_class_mappings
    WHERE card_name = card_record.card_data->>'name'
      AND card_type = card_record.card_type
    LIMIT 1;

    -- Get class multiplier based on card type
    class_mult := 1.0;
    IF card_class IS NOT NULL THEN
      IF card_record.card_type = 'hero' THEN
        SELECT health_multiplier INTO class_mult
        FROM class_multipliers
        WHERE class_name = card_class;
      ELSE
        SELECT health_multiplier INTO class_mult
        FROM dragon_class_multipliers
        WHERE class_name = card_class;
      END IF;
      
      IF class_mult IS NULL THEN
        class_mult := 1.0;
      END IF;
    END IF;

    -- Calculate final stats with rarity AND class multipliers
    final_health := ROUND(base_health * rarity_mult * class_mult);
    final_defense := ROUND(base_defense * rarity_mult * class_mult);
    final_power := ROUND(base_power * rarity_mult * class_mult);
    final_magic := ROUND(base_magic * rarity_mult * class_mult);

    -- Update card_instances with calculated stats
    UPDATE card_instances
    SET 
      max_health = final_health,
      max_defense = final_defense,
      max_power = final_power,
      max_magic = final_magic,
      -- Update current values proportionally if card was damaged
      current_health = CASE 
        WHEN card_record.old_health > 0 AND current_health = card_record.old_health THEN final_health
        WHEN card_record.old_health > 0 THEN ROUND(current_health::NUMERIC / card_record.old_health * final_health)
        ELSE final_health
      END,
      current_defense = CASE 
        WHEN card_record.old_defense > 0 AND current_defense = card_record.old_defense THEN final_defense
        WHEN card_record.old_defense > 0 THEN ROUND(current_defense::NUMERIC / card_record.old_defense * final_defense)
        ELSE final_defense
      END,
      -- Update card_data JSON with correct stats
      card_data = jsonb_set(
        jsonb_set(
          jsonb_set(
            jsonb_set(
              card_data,
              '{health}', to_jsonb(final_health)
            ),
            '{defense}', to_jsonb(final_defense)
          ),
          '{power}', to_jsonb(final_power)
        ),
        '{magic}', to_jsonb(final_magic)
      ),
      updated_at = NOW()
    WHERE id = card_record.id;

  END LOOP;
  
  RAISE NOTICE 'Card stats recalculated successfully with proper multipliers';
END $$;
