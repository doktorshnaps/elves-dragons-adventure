-- Patch: safe update function without legacy inventory
...
BEGIN
  ...
  IF NOT is_admin_wallet(p_wallet_address) THEN
    IF NOT EXISTS (
  ...
  IF user_record IS NULL THEN
    ...
    INSERT INTO public.game_data (
      ...
      COALESCE(p_wood_last_collection_time, (EXTRACT(epoch FROM now()) * 1000)::bigint),
      COALESCE(p_stone_last_collection_time, (EXTRACT(epoch FROM now()) * 1000)::bigint),
