-- Удаляем ВСЕ версии add_card_to_medical_bay с любыми сигнатурами
DROP FUNCTION IF EXISTS public.add_card_to_medical_bay(UUID, INT, TEXT);
DROP FUNCTION IF EXISTS public.add_card_to_medical_bay(TEXT, UUID, INT);
DROP FUNCTION IF EXISTS public.add_card_to_medical_bay(UUID, TEXT);
DROP FUNCTION IF EXISTS public.add_card_to_medical_bay(UUID);

-- Пересоздаем функцию с правильной сигнатурой
