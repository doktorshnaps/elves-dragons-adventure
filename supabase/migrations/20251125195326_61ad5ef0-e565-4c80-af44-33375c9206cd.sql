-- Исправляем функцию recalculate_all_card_stats чтобы она могла выполнять UPDATE

DROP FUNCTION IF EXISTS public.recalculate_all_card_stats();
DROP FUNCTION IF EXISTS public.admin_recalculate_all_card_stats(text);
...
BEGIN
  ...
  -- Перебираем все карты в card_instances
  FOR v_card IN 
    SELECT 
      id,
      card_type,
  ...
  LOOP
    ...
    v_power := FLOOR(v_base_power * v_rarity_mult * v_class_mult_power);
    v_magic := FLOOR(v_base_magic * v_rarity_mult * v_class_mult_magic);
      ...
      card_data = jsonb_set(
        jsonb_set(
          jsonb_set(
            jsonb_set(
              card_data,
              '{health}', to_jsonb(v_max_health)
            ),
            '{defense}', to_jsonb(v_max_defense)
          ),
          '{power}', to_jsonb(v_power)
        ),
        '{magic}', to_jsonb(v_magic)
      ),
      updated_at = now()
    WHERE id = v_card.id;
...
-- Создаем админскую обертку для безопасного вызова
CREATE OR REPLACE FUNCTION public.admin_recalculate_all_card_stats(p_admin_wallet_address text DEFAULT 'mr_bruts.tg')
RETURNS jsonb
LANGUAGE plpgsql
VOLATILE
SECURITY DEFINER
...
BEGIN
  ...
  RETURN public.recalculate_all_card_stats();
END;
$$;

-- Запускаем пересчет для всех существующих карт
SELECT public.recalculate_all_card_stats();