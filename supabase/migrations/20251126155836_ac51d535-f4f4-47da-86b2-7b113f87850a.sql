-- Исправление функции add_card_to_medical_bay для корректного расчета таймера
-- Проблема: если карта здорова, estimated_completion = now() и кнопка "Забрать" сразу активна
...
DECLARE
  ...
  v_heal_rate integer := 100; -- 100 HP per minute
...
BEGIN
  ...
  -- ✅ ИСПРАВЛЕНИЕ: Минимум 1 минута даже для полностью здоровых карт
  IF v_missing_hp = 0 THEN
    v_minutes_needed := 1; -- Минимум 1 минута вместо 0
  ELSE
  ...
  INSERT INTO public.medical_bay (
    ...
    v_heal_rate,
    false -- ✅ ВСЕГДА false при добавлении
