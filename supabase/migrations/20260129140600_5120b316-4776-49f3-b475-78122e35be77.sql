...
-- Recreate with race condition protection
...
BEGIN
  -- ❗ CRITICAL: Check if player already has an active bot match being created
  -- This prevents race condition when multiple requests come at the same time
  ...
  IF v_existing_match_id IS NOT NULL THEN
    -- Return existing match instead of creating duplicate
