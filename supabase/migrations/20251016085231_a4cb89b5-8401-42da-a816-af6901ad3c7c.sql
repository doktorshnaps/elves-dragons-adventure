-- Drop and recreate get_game_data_by_wallet_full function to fix 500 errors
...
BEGIN
  ...
  SELECT 
    ...
    CASE 
      ...
      ELSE gd.cards
    END as cards,
    -- Limit inventory to 200 items
    CASE 
      WHEN jsonb_array_length(COALESCE(gd.inventory, '[]'::jsonb)) > 200
      THEN (SELECT jsonb_agg(elem) FROM jsonb_array_elements(gd.inventory) WITH ORDINALITY AS t(elem, ord) WHERE t.ord <= 200)
      ELSE gd.inventory
    END as inventory,
