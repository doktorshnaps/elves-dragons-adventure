-- Drop and recreate admin_create_card_upgrade_requirement to include upgrade_time_hours
DROP FUNCTION IF EXISTS admin_create_card_upgrade_requirement(
  text, text, text, text, text, integer, integer, text, integer, integer, 
  integer, integer, integer, integer, integer, json, text, integer
);

CREATE OR REPLACE FUNCTION admin_create_card_upgrade_requirement(
  ...
  p_success_chance INTEGER DEFAULT 90,
...
-- Also update admin_update_card_upgrade_requirement to include upgrade_time_hours
DROP FUNCTION IF EXISTS admin_update_card_upgrade_requirement(
  text, uuid, integer, integer, integer, integer, integer, integer, 
  integer, json, text, integer
);

CREATE OR REPLACE FUNCTION admin_update_card_upgrade_requirement(
  p_wallet_address TEXT,
  p_id UUID,
  ...
  p_upgrade_time_hours NUMERIC DEFAULT NULL
)
RETURNS BOOLEAN
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  SET 
    ...
    required_building_id = COALESCE(p_required_building_id, required_building_id),
    required_building_level = COALESCE(p_required_building_level, required_building_level),
    upgrade_time_hours = COALESCE(p_upgrade_time_hours, upgrade_time_hours),
    updated_at = now()
  WHERE id = p_id;
