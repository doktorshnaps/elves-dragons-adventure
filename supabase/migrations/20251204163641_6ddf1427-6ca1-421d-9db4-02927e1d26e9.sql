-- Drop all existing versions of add_card_to_forge_bay
DROP FUNCTION IF EXISTS public.add_card_to_forge_bay(uuid, integer, text);
DROP FUNCTION IF EXISTS public.add_card_to_forge_bay(uuid, numeric, text);

-- Create single unified version with proper slot limit validation
CREATE OR REPLACE FUNCTION public.add_card_to_forge_bay(
  p_card_instance_id uuid,
  p_repair_hours integer DEFAULT 1,
