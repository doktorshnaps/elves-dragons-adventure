-- Create card_instances table if it does not exist
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.tables 
    WHERE table_schema = 'public' AND table_name = 'card_instances'
  ) THEN
...
-- Ensure unique instance per wallet+template
DO $$ BEGIN
  IF NOT EXISTS (
...
-- Update existing functions to ensure presence
