-- Clear all existing versions of the function to fix overloading conflict
-- We need to find all function signatures and drop them explicitly

-- Drop specific function signatures one by one to avoid conflicts
...
-- Drop any other potential variants
DROP FUNCTION IF EXISTS public.update_game_data_by_wallet(text);
DROP FUNCTION IF EXISTS public.update_game_data_by_wallet(text, integer);
DROP FUNCTION IF EXISTS public.update_game_data_by_wallet(text, integer, jsonb);
DROP FUNCTION IF EXISTS public.update_game_data_by_wallet(text, integer, jsonb, jsonb);
DROP FUNCTION IF EXISTS public.update_game_data_by_wallet(text, integer, jsonb, jsonb, jsonb);
DROP FUNCTION IF EXISTS public.update_game_data_by_wallet(text, integer, jsonb, jsonb, jsonb, jsonb);

-- Create one unified function with all needed parameters as optional (DEFAULT NULL)
