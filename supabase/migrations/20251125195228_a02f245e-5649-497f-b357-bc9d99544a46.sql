-- Функция для пересчета характеристик всех карт в card_instances с учетом мультипликаторов

CREATE OR REPLACE FUNCTION public.recalculate_all_card_stats()
RETURNS TABLE(
  total_updated integer,
  heroes_updated integer,
  dragons_updated integer,
  workers_skipped integer
)
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  LOOP
    ...
    -- Обновляем запись
    -- Сохраняем текущее здоровье и броню, но не больше новых максимумов
...
BEGIN
  ...
  RETURN QUERY SELECT * FROM public.recalculate_all_card_stats();
END;
$$;