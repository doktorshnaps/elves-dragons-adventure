...
-- Minimal policies: allow inserts/updates/selects only when wallet_address is present; reads aren't needed but useful for admin later.
-- We’ll rely on a SECURITY DEFINER function for app access to avoid RLS recursion issues.
create policy if not exists "Wallet owners can view wallet identities"
  on public.wallet_identities
  for select
  using (wallet_address is not null);

create policy if not exists "Wallet owners can insert wallet identities"
  on public.wallet_identities
  for insert
  with check (wallet_address is not null);

create policy if not exists "Wallet owners can update wallet identities"
  on public.wallet_identities
  for update
  using (wallet_address is not null);
...
-- Add FK and index
alter table public.wallet_connections
  add constraint if not exists wallet_connections_identity_id_fkey
  foreign key (identity_id) references public.wallet_identities(id) on delete set null;

create index if not exists idx_wallet_connections_identity_id on public.wallet_connections(identity_id);
