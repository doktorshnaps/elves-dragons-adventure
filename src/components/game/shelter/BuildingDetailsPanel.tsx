...
export const BuildingDetailsPanel = ({
  ...
  const detailsContent = <div className="p-6 space-y-6">
      ...
      {selectedBuilding.level < selectedBuilding.maxLevel && <>
          ...
          {/* Upgrade Button */}
          <Button className="w-full font-bold text-base py-6 transition-all duration-300 hover:-translate-y-0.5" onClick={onUpgrade} disabled={!canAfford && !isUpgradeReady || isUpgrading && !isUpgradeReady} variant={isUpgradeReady ? "default" : undefined}>
