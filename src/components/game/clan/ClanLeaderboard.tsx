...
import { ClanProfileModal } from './ClanProfileModal';
    ...
    <>
      <div className="space-y-2">
        {leaderboard.map((entry, index) => (
          <div
            key={entry.id}
            onClick={() => setSelectedClanId(entry.id)}
            className={`bg-black/40 backdrop-blur-sm border rounded-xl p-3 cursor-pointer hover:bg-white/5 active:scale-[0.99] transition-all ${getRankBg(index)} ${
      ...
      <ClanProfileModal
        ...
        open={!!selectedClanId}
        onOpenChange={(open) => { if (!open) setSelectedClanId(null); }}
      />
    </>
  );
};
