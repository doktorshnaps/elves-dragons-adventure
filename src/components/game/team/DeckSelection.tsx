    ...
    <div className="flex flex-col h-full space-y-3 sm:space-y-4 min-h-0">
      ...
      <section className="bg-game-surface/50 p-2 sm:p-3 rounded-lg border border-game-accent flex-shrink-0" aria-label="Выбранная команда">
        <h1 className="text-sm sm:text-base font-bold text-game-accent mb-2 sm:mb-3">
          Команда ({selectedPairs.length}/5)
        </h1>
                ...
                {pair ? (
                  <div className="space-y-1">
                    ...
                    <Button 
                      ...
                      className="w-full text-[10px] sm:text-xs h-6 sm:h-8 p-1"
                    >
                      Х
                    </Button>
                  </div>
                ) : (
                  <div className="h-full flex items-center justify-center text-game-accent/50 text-[10px] sm:text-xs text-center">
                    Пустой
                  </div>
                )}
              </div>
          ...
          <div className="grid grid-cols-4 sm:grid-cols-6 gap-3 overflow-y-auto p-4">
            {heroes.map(hero => {
              ...
              return (
                <div 
                  key={hero.id} 
                  className={`cursor-pointer transition-all ${isSelected ? 'opacity-50' : 'hover:scale-105'}`} 
                  onClick={() => canSelect && handleHeroSelect(hero)}
                >
                  <CardDisplay 
                    card={hero} 
                    showSellButton={false} 
      ...
      <Dialog open={showDragonDeck} onOpenChange={(open) => {
        ...
        <DialogContent className="max-w-4xl h-[80vh] bg-game-surface border-game-accent">
          ...
          <div className="grid grid-cols-4 sm:grid-cols-6 gap-3 overflow-y-auto p-4">
            {(activePairIndex !== null 
              ...
              const canAssign = activePairIndex !== null 
                ? !!selectedPairs[activePairIndex] 
                  ...
                  && (selectedPairs[activePairIndex]?.hero.rarity ?? 0) >= dragon.rarity 
                  && !isSelected 
                : false;
              return (
                <div 
                ...
                >
                  <CardDisplay 
                    ...
                    onClick={(e) => {
                      ...
                      const canAssignHere = activePairIndex !== null 
                        && !!selectedPairs[activePairIndex] 
                        && !selectedPairs[activePairIndex]?.dragon 
                        && selectedPairs[activePairIndex]?.hero.faction === dragon.faction 
                        && (selectedPairs[activePairIndex]?.hero.rarity ?? 0) >= dragon.rarity 
            ...
            {activePairIndex !== null 
              && getAvailableDragons(selectedPairs[activePairIndex]?.hero.faction, selectedPairs[activePairIndex]?.hero.rarity).length === 0 
              && (
