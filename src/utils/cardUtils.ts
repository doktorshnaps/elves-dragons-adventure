...
export const getClassDropRates = async () => {
  ...
  const heroes: Record<string, { name: string; chance: string }> = {};
  const dragons: Record<string, { name: string; chance: string }> = {};
...
export const generateCard = (type?: CardType): Card => {
  ...
  if (!type) {
    ...
    cardType = type;
    console.log(`🎯 Fixed Type: ${cardType === 'character' ? 'Герой' : 'Дракон'}`);
  }
  ...
  console.log(`✨ Generated Card: ${selectedCard.name} (${cardType}) ${rarity}⭐`, stats);
